...
fh = newfigure('histograms',[1 2]);

% Dimensions to plot
ds = [1 2 3 4 8 32 128 1024];

% Generate perceptually even light gray -> black ...
cols{1} = flip(makeColorSaturationSeries([0 0 0],numel(ds)));
cols{2} = cols{1};
cols{2}(:,1) = 1; % ... and light red -> red line colors
...
for i = 1:numel(ds)
   ...
   for t = 1:2
      ...
      axtivate(t);
      %plot(bins,h/sum(h),'Color',cols{t}(i,:),'LineWidth',2);
      plot(bins(2:end-1),h(2:end-1)/max(h),'Color',cols{t}(i,:),'LineWidth',1);
   end
end
